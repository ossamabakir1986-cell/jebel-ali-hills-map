/* Hayat GIS v3.3.8 - Add Inventory Direct Click Fix
   - Adds a checkbox to show/hide Add Inventory pins.
   - Makes the whole Add Inventory pin directly clickable (opens editor without popup).
...
(function(){
  ...
  function refresh(){
    ...
    (window.JAH_PA_LABELS||[]).forEach(function(pa){
      ...
      var mk=L.marker([lat,lng],{icon:icon,pane:'hayatAddInventoryPane',interactive:true,keyboard:false,bubblingMouseEvents:false,zIndexOffset:12000});
      // Direct click opens the Add Inventory editor. This is more reliable than requiring a popup button.
      ...
      mk.bindTooltip('Add inventory for '+label, {direction:'top', opacity:0.92, sticky:true});
      mk.addTo(layer);
      count++;
    });
    layer.addTo(map);
  ...
  css.textContent='\
  .hayat-add-inventory-pill{pointer-events:auto!important;cursor:pointer!important;}\
  .hayat-add-inventory-pill .pa-code,.hayat-add-inventory-pill .pa-plus{pointer-events:auto!important;}\
